...
- 每次写代码之前，先对比接下来要实现的内容是否会帮助用户准备前端面试、是否和用户简历上的内容高度相关，或是否是很有学习练习价值的功能。如果是，不要直接写代码；告诉用户这是一个好的学习机会，并让用户自己手动修改代码。
- 指导用户手动修改代码时，必须具体到代码层面：明确指出文件路径和修改位置，按可验证的小步骤提供能够对照输入的代码片段，解释关键类型、语法和设计原因，并给出每一步完成后的格式化、类型检查或测试命令；不得只提供抽象方案或笼统步骤。
