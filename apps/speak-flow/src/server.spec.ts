...
describe('chat API memory retrieval', () => {
  ...
  it('requires an explicit topic before injecting private pet memories', async () => {
    const userId = 'explicit-memory-topic-test-user';
    memoryStore.saveExtractedMemories(
      userId,
      [
        {
    ...
    const fetchMock = vi
      ...
      .mockImplementation(async (input, init) => {
        ...
        if (body.temperature === 0.8) {
          deepSeekPrompt = body.messages?.[0]?.content ?? '';
          return new Response(
            JSON.stringify({ choices: [{ message: { content: 'Okay.' } }] }),
            { status: 200 },
          );
        }
        return new Response(
